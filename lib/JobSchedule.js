...
class JobSchedule extends Parse.Object {
  ...
  run(date) {
    console.info("Running" + this.jobName);
    const options = {};
    options.headers = {
    ...
    return request(options)
      .then(() => {
        console.info("Successfully ran " + this.jobName + "!");
        const lastRunDate = new Date(date);
        lastRunDate.setMilliseconds(0);
        lastRunDate.setSeconds(0);
        return this.save({
          lastRun: Math.round(lastRunDate.getTime() / 1000)
        }, {useMasterKey: true});
      })
      .catch(res => {
        console.error("error running " + this.jobName, res.error);
        throw res;
      });
  }
